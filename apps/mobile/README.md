...
- 工位间最短合法交接：提交人直接走到相邻审核工位，双方保持无碰撞交流距离；
- 工位和前台员工使用相机匹配坐姿与椅背前景遮挡，休息员工使用沙发专用坐姿；
- 办公室内以独立“新任务”按钮和可收起的秘书工作台承载主要操作与导航；
- 新建任务 Sheet，可选择快速、标准或深度模式；
- 新建任务通过本地服务端 PoC 调用 DeepSeek V4 Flash / Pro，并在原生结果页展示正式答案；
- 员工档案包含人格、属性、技能和协作方式；视觉多人协作不伪造多次模型调用；
- iOS 15.1+，React Native New Architecture 与 Hermes。

## 当前架构边界

- 任务、AI、人事、时间、权益和账务最终以服务端事实为准；
- DeepSeek API Key 只存在于 `services/api` 的进程环境，移动包不包含供应商凭据；
