...
用户操作“直接汇报给老板”只提交视觉压缩意图，不修改 `Task.status`、`Task.ai_status` 或 AI Worker 调度。若 AI 尚未完成，Projection 将视觉焦点转到最终负责人并保持处理中状态，通过原生状态气泡继续承接等待；只有收到 `output.ready` 后才进入规定的 5 秒交付收束流程。
