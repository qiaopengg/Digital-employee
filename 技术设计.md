...
> 核心结论：采用“React Native 单客户端 + RN 原生交互层 + Skia 角色与道具层 + 确定性运动学办公室 + 云端耐久任务编排”。
...
| 场景静态与交互层 | RN `View` / `Image` / `Pressable` | 分层背景、家具、前景遮挡、状态图标、工作气泡、点击热区和无障碍 |
| 角色与道具绘制 | React Native Skia `Atlas` | 多方向序列帧人物、随身道具、批量变换和动态深度 |
| 动画与视图过渡 | Reanimated 4.5.3、Worklets 0.11.1 | UI 线程运动学、距离步态、房间聚焦和秘书总控过渡 |
| 局部动效候选 | Rive | 不进入首版人物主链路；只在独立 PoC 证明收益后用于局部动效 |
...
产品需要的是竖屏、轻量、可读、可点击的互动办公室，而不是自由空间模拟。固定房间、有限姿态、语义槽位、显式遮挡和受控并发由同一 React Native 客户端承载；角色与道具使用 Skia 画布批量绘制，文字、状态、导航、支付、文件和无障碍继续使用原生 RN 层，共享导航、生命周期和业务状态。

当前方案明确：

- 只维护一个 React Native 客户端和一套 TypeScript 场景状态；
- 不使用 WebView、Three.js 或额外场景运行时；
- 冻结当前办公室镜头角度、方向和透视，不通过换机位、人物缩放或遮挡规避穿帮；
- 不支持自由旋转、通用刚体动力学或任意家具摆放；使用确定性运动学、静态碰撞、路径时空预约和接触点约束实现可信物理逻辑；
- 场景点击只产生用户意图，不能直接改写任务、人事、资产或账务；
- Skia `Atlas` 是首版人物与随身道具的默认绘制层，不承担页面、业务状态或无障碍；
- 正式结果、长文、支付、文件、Sheet、卡片、状态图标、工作气泡和无障碍全部由原生 RN UI 承担；
- 对话气泡只在真实工作对接、交接、复核或汇报期间显示；常态状态使用图标、颜色和按需文字。

### 1.3 版本冻结原则

- React Native 固定在 0.86.x 的已验证补丁，启用 New Architecture 与 Hermes；
- Reanimated 与 Worklets 已按包级依赖约束锁定为 4.5.3 / 0.11.1，两者明确覆盖 RN 0.86；必须在双平台动作 PoC 中继续验证原生构建与前后台恢复；
- Skia 已锁定为 2.9.1，满足 RN ≥0.79、React ≥19；动作 PoC 后记录双平台安装步骤、包体变化和回归矩阵；
- Rive 不作为首版人物依赖；若后续启用，必须单独评审 Nitro Modules、运行时版本、资源管线、包体、并发和退出成本；
...
单客户端与人物动作路线的工程可行性信心评估约 **95%**：RN 0.86 的兼容版本范围、Skia 精灵批量绘制、Reanimated UI 线程逐帧更新、路径预约、接触点和降级边界均已明确。剩余风险是动作资产质量、纹理驻留内存和低端真机帧时间，必须由《办公室角色动作与场景物理技术方案.md》的垂直 PoC 和 Release 真机 Gate 证明。该比例不是发布成功率，也不代表未经验证的视觉质量已经合格。
...
4. **交付收束有硬上限**：客户端收到 `output.ready` 后必须完成规定的压缩交付演出，并在 5 秒内开放结果入口；超时或资源异常时立即收束为等价完成状态。
...
16. **仿真服从可用性**：系统只投影可理解的职场流程与因果关系；职场演出不得阻断 AI 连续执行、越过安全边界或突破效率预算，发生冲突时使用明确的产品替代规则。
...
办公室首版提供临时工作点、小型独立办公室和标准公司楼层三套竖屏基础模板，共享同一语义槽位和投影协议；其中最复杂的标准公司楼层包含 3—4 个房间。先以标准公司楼层、4 名正式员工和 6 个核心姿态完成一套黄金样板，签字后才制作其余两套。全景重点显示 4—6 人，当前楼层最多 8 人，同时移动最多 3 人。

首版不实现自由空间编辑、通用刚体动力学、自由旋转、持续后台模拟、多个智能体自由对话、真实发送邮件或发布内容、大型开放世界、多人实时协作，也不将 Redis、客户端缓存或 Temporal 历史当作业务主数据库。首版仍必须实现家具不可穿越、人物不相撞、脚步与位移一致、座椅 / 沙发接触和道具唯一所有者等确定性物理约束。
...
当前仓库只保留 `apps/mobile` 一个客户端；其余目录按对应 Gate 开始时逐步创建，
不为尚未实施的领域提前建立空壳。项目不得重新引入第二客户端工程或场景专用
业务接口。构建产物、缓存、临时导出物和真实用户文件不得提交版本库。

## 4. React Native 客户端与办公室

### 4.1 信息架构与页面边界

顶级信息架构保留：**办公室、任务、公司、汇报、我的**。办公室是默认首屏，该路由使用沉浸式 `OfficeShell`，导航冻结为独立 `NewTaskButton` 与屏幕边缘 `SecretaryDock`。前者始终一步可达；后者收起时只显示秘书图标与待办数，展开后进入任务、公司、汇报、我的及其他管理能力，不常驻完整 Tab bar 或横向多按钮栏。登录恢复、新手引导、任务创建和列表、项目、审批、正式结果、来源、文件、人事、财务、支付、隐私、导出和删除全部是 RN 页面。

办公室场景负责空间叙事、员工状态、任务道具、房间聚焦和意图入口。原生 UI 负责文字、状态图标、工作交接气泡、卡片、Sheet、长文、支付、文件、系统分享和无障碍。任何场景点击都只返回 `SceneUserIntent`，由路由或命令确认层处理；点击员工、房间或任务不能直接改变业务状态。
...
- `actors-and-props`：Skia Atlas 绘制的透明员工、任务文件、电话和有限动态道具；
- `front-mask`：前墙、前景家具、门框和遮挡切片；
- `native-overlays`：按需姓名、状态图标、工作交接气泡、热区、秘书总控、Sheet 和无障碍节点。

每个元素必须显式声明 `zIndex`。路径节点携带遮挡区段或层级提示；员工移动时按当前路径节点和目标槽位计算层级，不使用隐式数组顺序。文字与交互热区不得烘焙进背景。

### 4.3 资源格式基线

- 背景与静态家具可使用经 iOS / Android 真机验证的 JPEG、PNG 或静态 WebP；
- 人物与随身道具使用带透明通道的多方向 Sprite Atlas，并提供静态姿态回退；动画 WebP 不作为人物动作基线；
- 所有动作片段共享同一根锚点与世界尺度，manifest 记录帧矩形、方向、帧率、步幅、脚部接触区间、手部 / 道具事件和合法衔接；
- 人物根运动、距离步态、转向和房间聚焦由 Reanimated UI worklet 驱动；Skia 负责人物与道具绘制；JS 线程只处理语义状态和低频事件；
- Rive 不进入首版人物主链路；启用前必须以独立 PoC 证明固定俯视动作、距离步态、家具接触、多实例性能和资源成本优于 Atlas；
...
```ts
type OfficeFacing = 'n' | 'ne' | 'e' | 'se' | 's' | 'sw' | 'w' | 'nw';
...
type OfficePath = {
  ...
  fromSlotId: string;
  toSlotId: string;
  nodes: Array<{ x: number; y: number; zIndex: number; facing?: OfficeFacing; waitSocketId?: string }>;
};
...
服务端发送语义槽位、真实安全状态和 `visual_intent`。客户端使用 `layout_version` 查找 `OfficeSlot` / `OfficePath`，映射 normalized x/y、冻结 scale、zIndex、facing、labelAnchor、interaction sockets 和 supportedPoses。具体动作片段、脚步相位、路径预约、家具接触和道具所有者变化仅在客户端按版本化 manifest 计算，详见《办公室角色动作与场景物理技术方案.md》。布局版本未知、路径缺失或姿态不支持时，回退到安全槽位和静态姿态，并请求完整快照。
...
没有另一套运行时需要暂停。结果页本身是普通 RN 路由，场景组件卸载或失焦即可停止动画。`output.ready` 前不得提前走向汇报槽位或显示完成徽标；客户端收到 `output.ready` 后立即压缩剩余动作，在 5 秒内完成整理、复核、汇报并开放结果入口。动画超时、资源缺失或渲染降级时直接应用等价完成状态，仍不得突破 5 秒上限。多个完成任务由秘书汇报收件箱聚合，不连续强制跳转。

用户操作“直接汇报给老板”只提交视觉压缩意图，不修改 `Task.status`、`Task.ai_status` 或 AI Worker 调度。若 AI 尚未完成，Projection 将视觉焦点转到最终负责人并保持工作姿态，通过状态图标与点击后的任务卡 / Sheet 承接等待；没有真实工作对接事件时不显示对话气泡。只有收到 `output.ready` 后才进入规定的 5 秒交付收束流程。

员工汇报页至少包含员工身份与两行内 `PersonaReport`、权威 `WorkProduct`、来源、时效、限制、未解决问题、交付物引用，以及复制、系统分享、导出、追问和退回修改。虚拟结算展示在正式结果之后，不遮挡答案。

追问、格式调整和普通修改沿用原 `task_id` 与结算单元，但必须创建新的修订执行尝试并重新派单；新尝试可以复用原上下文和团队配置，仍需产生独立状态、用量、版本和视觉流程记录。
...
任务 Workflow 根据复杂度、风险和验证要求选择执行模式：简单、低风险任务默认使用单次真实模型调用；复杂、高风险或明确要求独立复核的任务可以升级为多个真实执行节点、模型调用或工具验证。真实执行节点必须记录可追踪事件、中间产物引用和实际用量，并纳入同一任务的成本预占、结算与审计。

数字员工与真实执行节点采用多对多映射，不强制一名员工对应一次模型调用。每个任务的完整视觉流程默认投影为多人参与的办公室流程，视觉组织路径表现为管理者自上而下安排、执行者处理、结果自下而上复核和汇报；这种演出不得反向创建 AI 执行事实，也不得把单次调用描述成多个独立 Agent 的分析或复核。
...
AI Gateway 只发布可验证事件：`execution.queued`、`model.started`、`tool.started`、`tool.completed`、`tool.failed`、`file.parsing`、`file.parsed`、`generation.streaming`、`validation.started`、`validation.completed`、`execution.blocked`、`execution.resumed`、`output.ready`、`execution.failed` 和 `execution.canceled`。只有对应事件存在时，Projection 才能在任务卡 / Sheet 中生成搜索、审核、计算、文件处理或来源核对等具体操作说明；无法获得细分进度时只显示人格化的通用处理中，不猜测具体操作、完成步骤或百分比。场景对话气泡仍只用于真实工作对接、交接、复核或汇报。
...
App 内持续数字服务只通过 App Store IAP 自动续期订阅销售。产品不创建虚拟货币包、办公资产包或以真实货币补充公司余额的 IAP 商品。客户端使用 StoreKit 2 和不可逆推导账号身份的 `appAccountToken`；服务端通过 App Store Server Notifications V2 与 Server API 维护权威订阅、退款、撤销、宽限期、过期和恢复购买。供应商成本账本、用户额度账本、虚拟公司资金账本三者分离。
...
人事申请与生效命令分离。秘书是永不离职、永不请假且始终可用的系统基础岗位，不进入请假、离职或岗位待补状态。其他员工的离职、解雇、调岗和请假在事务或 Saga 中更新任务交接、权限、工位槽位和工资；员工不可用时默认把任务代管关系转给秘书，由秘书重新派单或在用户明确操作后发起远程召回，不暂停底层 AI。离职生效后幂等创建对应岗位的招聘与面试流程，最终录用等待用户决定；新员工入职前岗位保持“待补”，秘书只代管职责，不伪造在岗员工。最终离职必须有截止和升级路径。
...
`FinanceTransaction` 是余额权威；`TaskSettlement` 使用稳定 `settlement_unit_key`，同一单元只有一条成功记录。收入、费用、工资应付和现金付款分开记账；发薪只结清应付；更正使用冲正，不删除历史。

任务复杂度由服务端版本化分类器写入，输入只包括任务类型、目标交付物、文件与数据处理范围、依赖与里程碑、所需工具、验证要求、风险等级和截止条件等可解释字段。客户端和用户不能提交权威复杂度；快速 / 标准 / 深度模式只影响 AI 路由、质量、速度和额度，不进入虚拟收入复杂度。分类结果保存 `complexity_rule_version` 与 `complexity_factors`，只有明确扩大范围或新建里程碑才允许生成新判定记录。

虚拟项目计算收入是由 `settlement_rule_version`、任务类型、服务端复杂度、时效、里程碑和合同等规范化输入决定的确定性服务端函数，禁止使用随机数、客户端金额、输入字数、输出字数或 Token 数量。字数与 Token 只进入能力限制、用量和供应商成本账本。相同规则版本和相同输入必须得到相同 `calculated_award`，并保存完整 `calculation_breakdown`；重试和重复提交复用原 `settlement_unit_key`，不得重新计算以获取更高收入。

虚拟项目收入按公司时区的自然日执行硬上限。结算事务锁定 `(company_id, local_date, income_rule_version)` 对应的每日收入计数器，使用 `actual_award = min(calculated_award, max(0, daily_cap - settled_today))` 计算实际收入，再原子写入 `TaskSettlement`、`FinanceTransaction` 和计数器。达到上限后任务仍正常执行和交付，结算记录保存 `daily_income_cap_reached` 原因，不产生收入交易；设备时间、并发、重试和跨设备操作不得创建第二个当日计数器。

每周基础经营周转金由公司时区调度器按 `(company_id, local_week, rule_version)` 幂等执行，只在权威虚拟余额低于 `minimum_operating_reserve` 时写入差额型 `operating_safety_net` 交易。余额已达生存线时不创建交易，未发放差额不累计。资产购买、扩建和升级命令使用 `discretionary_spendable_balance = max(0, balance - minimum_operating_reserve)` 校验可支配余额，保护准备金仍属于同一本虚拟资金账，不创建受限代币或第二余额。

办公场所、家具、设备、设施和装饰统一建模为 `CompanyAsset`，复用 Economy / Ledger，不创建独立装修余额。租赁、购买、维护、折旧、维修、出售、搬迁和处置命令必须幂等并引用 `FinanceTransaction`；办公室搬迁、关闭房间和高价值资产处置需要用户确认。Projection 只读取生效的办公模板、语义槽位、资产状态与场景版本，不得通过动画反向修改资产所有权或账本。

办公模板共享同一套语义对象协议，不为每套场景复制任务和员工状态机。家具与设备只放入模板声明的预设槽位；经营危机时可以切换到更小的已授权模板、停用房间或显示资产搬迁状态，但不得影响订阅权益、AI 调度或正式 `WorkProduct`。办公场景、家具、设备和装饰只允许通过权威虚拟资金账本取得，不接收 IAP 或其他真实付款结果作为资产购买依据。
...
│   ├── CompactOfficeHUD
│   ├── OfficeViewport
│   │   ├── RN Background / Floor / RearFurniture
...
`OfficeViewport` 只消费可序列化的场景视图模型。Skia 画布不承载业务文字、点击语义或无障碍；原生 Overlay 提供透明热区、状态图标、工作交接气泡与场景等价入口。组件不得调用模型、修改任务状态或直接写账本。

### 7.2 固定镜头、布局与家具

- 画布使用 normalized 坐标适配安全区与不同屏幕比例；
- 当前前方高位俯视镜头的角度、方向、透视和设计画布冻结；房间聚焦只能在该构图上做受限平移 / 放大，不允许切机位；
- 办公室首页的 `OfficeViewport` 占用安全区内除紧凑 HUD 外的主要空间；`SecretaryDock` 收起时不参与场景主内容布局，不用空白占位保留展开高度；
- 全景默认容纳所有 3—4 个房间和 4—6 名重点员工；
- 模板按目标容量声明固定工位；员工不足时工位保持空置，不重排办公室或复制人物；
- 布局 manifest 先声明功能分区、容量、工位协作组、主通道、房间入口、家具占地、碰撞体、接触点和前景遮挡，再生成 `OfficeSlot` 与 `OfficePath`；
- 家具只能进入通过验证的语义摆放槽位；每件可互动家具声明 approach、seat / hand / prop contact、front mask、capacity 和 allowed clips；
- P0 不依赖双指缩放或拖拽；点击员工或房间以 Reanimated 在冻结镜头中完成受限聚焦并提供“回到全景”；
- 路径、家具、坐姿和遮挡问题必须在冻结镜头下修复，不能通过人物 scale、透明度重影或额外遮挡掩盖。

### 7.3 确定性人物运动与互动

首版使用八方向 Sprite Atlas。语义姿态与动作片段分离；动作 manifest 记录帧矩形、方向、根锚点、固定世界尺度、步幅、脚部接触区间、手部 / 道具事件和合法衔接。人物世界比例在同一模板内保持不变，禁止 `scaleY` 起身、透明度重影和两帧步态。

行走相位由累计路程除以片段步幅计算；起步、收步、转身和交互使用独立片段。朝向与移动方向不一致时先停止根平移并转身，禁止倒退行走。静态家具使用人物半径扩张后的连续路径碰撞检查；最多 3 名移动人物使用节点、边和目标槽位的时间预约处理对向通行与让行，不引入刚体响应或首版 RVO。

工位和沙发分别定义座位根点、站立点、出入口、朝向、手脚接触和前景遮挡，并使用各自专用坐下 / 坐姿 / 起身片段。工作交接默认到接收者工位的 visitor socket；文件在动作关键帧切换唯一视觉所有者。完整算法、状态机、资源协议与 Gate 见《办公室角色动作与场景物理技术方案.md》。

### 7.4 工作时间行为与场景 UI

- 工作时间内，在岗员工没有移动事件时默认坐在自己的固定工位；
- 饮水、如厕、短交流和伸展具有目的地预约、最晚返回时间、冷却、往返路径和任务中断规则；不允许长期站立和随机游走；
- 对话气泡只在真实工作对接、交接、复核或汇报期间显示并绑定 `work_event_id`；
- 常态状态使用轮廓不同的原生图标与语义色，点击 / 聚焦、VoiceOver 和场景等价列表提供文字；
- 独立 `NewTaskButton` 始终一步可达，`SecretaryDock` 收起时只显示图标与待办数；
- 路径缺失、目标不可达、资源失败或减少动态效果开启时，停止动作并应用安全静态姿态 / 等价状态，不发业务完成命令。

Skia 绘制层和静态 RN 回退必须消费同一 `OfficeSlot`、`OfficePath`、`EmployeeSceneState`、`SceneSnapshot` 与 `visual_intent`，保持同一产品事实和原生交互能力。
...
每个资源记录稳定 `asset_id`、源文件引用、导出格式、像素尺寸、倍率、色彩空间、透明边界、压缩方式、目标平台、许可、作者、来源、到期或限制。每个角色动作片段记录帧矩形、八方向 facing、根锚点、固定世界尺度、步幅、脚部接触区间、手部 / 道具事件、合法前后片段和支持员工集合。每个家具记录 footprint、collider、approach / contact sockets、front masks、capacity 与 allowed clips。每个槽位记录房间、语义类型、normalized x/y、冻结 scale、zIndex、facing、labelAnchor、supportedPoses；每条路径记录节点、让行点、方向 / 容量、层级和遮挡穿越信息。
...
CI 至少检查：尺寸与倍率、格式白名单、透明像素边界、根 / 脚 / 手 / 道具锚点、动作尺度一致性、步幅与接触区间、片段衔接、家具接触点、槽位范围、路径端点与 collider、显式 zIndex、遮挡区域、未引用资源、重复 ID、许可缺失或过期、压缩预算、哈希和 manifest Schema。P0 错误阻止合并与发布。公共资源与用户业务数据使用不同存储前缀和权限。
...
| 结果可见 | 客户端收到 `output.ready` 后 ≤5 秒完成交付演出并开放结果入口 |
...
6. 检查状态图标、工作气泡和无障碍节点是否造成布局抖动；
7. 检查 Atlas 分包、不可见 clip 释放、纹理重复驻留和 Skia draw call；
8. 使用完全相同夹具复验，不降低人数、动画或业务能力来伪造通过。
...
真实性测试必须证明：AI 未开始时不显示搜索或验证完成；等待用户时停止生成类表达；系统失败不归咎员工；客户端收到 `output.ready` 后 5 秒内完成交付演出并开放结果入口；断线不补播旧动作；同一员工不处于两个槽位；同一任务不生成两个主要道具；减少动态效果或资源缺失时仍有正确文字状态。
...
实现 RN 0.86.x 单客户端、一个冻结镜头的竖屏办公室、3—4 个房间、4 名正式员工、固定空工位、6 个语义姿态、日夜、全景与聚焦；先完成两名员工在工位间的一次完整 Skia Atlas 交接，再完成一个真实文本任务驱动接单、工作、交接、整理、走向汇报槽位和原生结果页；验证人物尺度、路径、脚步、家具接触、状态图标、工作气泡、减少动态效果、授权与视觉签字。

### 15.2 阶段 0B：性能压力

使用 RN 原生交互层与 Skia `Atlas` 角色 / 道具层实现 4 房间、8 员工、3 并行动画、1 个工作交接气泡、2 个展开任务标签的 20 分钟 Release 真机夹具；完成 50 次前后台和快照恢复。按第 9.3 节做两轮定向优化并用相同夹具复验；若仍失败，不扩展人物资产，先调整纹理、动作并发或实现方式并记录风险接受。
...
扩展 IAP、权益、成本、三类任务、文件、3 名核心员工与秘书、HR、工资、资产、项目、知识、导出、删除和无障碍；在黄金样板签字后完成临时工作点、小型独立办公室和标准公司楼层三套模板及其搬迁闭环。每增加领域，先完成状态、失败、权限、幂等、埋点和测试，再增加演出。发布前冻结员工、房间、姿态、事件和模板，完成设备矩阵、成本对账、安全、恢复、商店配置、灰度和回滚演练。
...
| Skia 角色层压力失败 | 8 人 / 3 动画不达 Gate | Atlas 分包、按需驻留、UI 线程采样、两轮同夹具优化；失败则停止扩展资产 |
...
2. ADR-002：RN 原生静态 / 交互层与 Skia Atlas 角色 / 道具层组成单一办公室；
3. ADR-003：语义槽位、固定导航图、时空预约、场景快照和视觉意图；
4. ADR-004：确定性运动学、距离步态、动作接触点和道具唯一所有者；
...
15. ADR-015：冻结办公室镜头、固定空工位、家具语义摆放槽位与工作时间返岗规则。

架构变化必须记录现状、问题、候选、决定、后果和回滚。新依赖说明维护、许可、包体、安全与退出成本；状态机、账本或权限变化同步需求、Schema、迁移、事件和测试；监控、删除和幂等不能上线后补。

## 20. 后续专项文档

已产出：《办公室角色动作与场景物理技术方案》。后续建议依次产出：《RN 场景 PoC 与真机验收表》《OfficeSlot / OfficePath / SceneSnapshot 协议》《scene manifest 与 2D 资源规范》《任务状态机与 Temporal 恢复》《Outbox、WebSocket 与投影》《AI 供应商、IAP、成本、Prompt 与评估》《PostgreSQL 账本与幂等》《文件与删除》《账号、隐私、支付与威胁模型》《现实时间与工资》《端到端发布验收》。
...
- [Reanimated 与 React Native 兼容矩阵](https://docs.swmansion.com/react-native-reanimated/docs/guides/compatibility/)
...
- [React Native Skia 安装与版本要求](https://shopify.github.io/react-native-skia/docs/getting-started/installation/)
- [React Native Skia Atlas](https://shopify.github.io/react-native-skia/docs/shapes/atlas/)
- [Rive React Native 新运行时迁移](https://rive.app/docs/runtimes/react-native/migration-guide)
- [Rive IK Constraint](https://rive.app/docs/editor/constraints/ik-constraint)
- [AAAI Cooperative Pathfinding](https://ojs.aaai.org/index.php/AIIDE/article/view/18726)
- [Unreal Engine Root Motion](https://dev.epicgames.com/documentation/unreal-engine/root-motion-in-unreal-engine)
...
首版以 React Native 单客户端承载全部页面和固定镜头的分层 2D 互动办公室。RN 原生层负责背景、家具、导航、状态图标、工作气泡、热区和无障碍，Skia `Atlas` 负责人物与随身道具，Reanimated worklet 负责确定性根运动和距离步态。人物只沿固定导航图与时空预约移动，工位、沙发和交接使用接触点与动作事件，不接入通用刚体动力学。服务端继续负责所有真实任务、人事、时间、权益和账务事实，Temporal 负责耐久流程，PostgreSQL 负责一致性与审计。主观美术由黄金视觉签字控制，工程结果由动作 PoC、Release 真机性能、生命周期、安全、IAP、成本、文件和恢复门槛共同决定。
